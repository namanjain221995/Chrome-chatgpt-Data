...
| 13 | Never `StrictHostKeyChecking=no`; pin host keys | `.github/actions/ec2-ssh/action.yml:105` (`yes`, unconditional), `:64-85` (pin, else trust-on-first-use with `::warning::`) | **Enforced.** Repo-wide grep finds no `=no`. The pin itself is optional in practice — an unpinned first run trusts whatever answers |
